...
parser.add_argument('--wd', '--weight-decay', default=0., type=float,
                    ...
                    dest='weight_decay')
parser.add_argument('-p', '--print-freq', default=49, type=int,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if ((epoch+1) % 10 == 0) | (epoch == 0):
            acc_log.append(acc1.detach().cpu().numpy())
    ...
    print (' * Best Final Acc@1:%.3f'%best_acc1)
    x=np.arange(0,len(acc_log)*10,10)
    plt.plot(x,acc_log)
    plt.xticks(np.arange(min(x), max(x)+1, 10))
    plt.xlabel('Epoch')
    plt.ylabel('Top-1 Acc(%)')
    plt.title('Test Accuracy')
    plt.savefig(os.path.join(save_root_path, logdir, 'TestAcc.png'), bbox_inches='tight',dpi=200)
    #plt.yticks((0,100))
    plt.savefig(os.path.join(save_root_path, 'TestAcc.png'),dpi=200)
